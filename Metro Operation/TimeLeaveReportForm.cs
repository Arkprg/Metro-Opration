...
namespace Metro_Operation
{
    ...
    public partial class TimeLeaveReportForm
    {
        public TimeLeaveReportForm()
        {
            InitializeComponent();
        }
        ...
        public void TimeLeaveReportForm_HelpButtonClicked(object sender, System.ComponentModel.CancelEventArgs e)
        {
            Help.ShowHelp(this, AppState.DatabasePath + "\\Help\\help.chm", HelpNavigator.TopicId, "040502");
        }

        public void Form31_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (OleDbConnection StrConnec=new OleDbConnection(AppState.ConnectionString))
                    {
                        ...
                        string StrQuer="Select Person.Fname, Person.Family, Person.P_Num, Person.Line_Num, Amal.Tarikh, Amal.Amal, Amal.U_Reg, Amal.T_Reg From Amal INNER JOIN Person ON Person.P_Num=Amal.P_Num WHERE Amal.Vis= True And Amal.OnvAmal='ساعتی'";
                        ...
                        {
                            ShowGridView.Rows.Add(i + 1, TempTable.Rows[i]["Fname"].ToString(), TempTable.Rows[i]["Family"].ToString(), TempTable.Rows[i]["P_Num"].ToString(), TempTable.Rows[i]["Tarikh"].ToString(), TempTable.Rows[i]["Amal"].ToString(), TempTable.Rows[i]["U_Reg"].ToString(), TempTable.Rows[i]["T_Reg"].ToString());
                        }
                        TempTable.Dispose();
                    }
