...
namespace Metro_Operation
{
    ...
    public partial class OvertimeReportForm
    {
        ...
        public void OvertimeReportForm_HelpButtonClicked(object sender, System.ComponentModel.CancelEventArgs e)
        {
            Help.ShowHelp(this, AppState.DatabasePath + "\\Help\\help.chm", HelpNavigator.TopicId, "040301");
        }

        public void OvertimeReportForm_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        public void ShowButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Enabled = false; //تکنسیسن

                    string StrQuer = "Select Person.Fname, Person.Family, Person.P_Num, Person.Line_Num, Ezafeh.Tarikh, Ezafeh.E_Time, Ezafeh.E_Kind, Ezafeh.E_Local, Ezafeh.E_Mem, Ezafeh.U_Reg, Ezafeh.T_Reg FROM Ezafeh INNER JOIN Person ON Person.P_Num=Ezafeh.P_Num WHERE Ezafeh.Vis=True AND Ezafeh.Tarikh BETWEEN '" + StartCalendar.Text + "' AND '" + EndCalendar.Text + "'";
                    ...
                    StrQuer += " ORDER BY Ezafeh.Tarikh DESC, Person.Family, E_Time";
                    ...
                    using (OleDbConnection StrConnec = new OleDbConnection(AppState.ConnectionString))
                    {
                        ...
                        {
                            ShowGridView.Rows.Add(i + 1, TempTable.Rows[i]["Fname"].ToString(), TempTable.Rows[i]["Family"].ToString(), TempTable.Rows[i]["P_Num"].ToString(), TempTable.Rows[i]["Tarikh"].ToString(), TempTable.Rows[i]["E_Time"].ToString(), TempTable.Rows[i]["E_Local"].ToString(), TempTable.Rows[i]["E_Kind"].ToString() + " " + TempTable.Rows[i]["E_Mem"].ToString(), TempTable.Rows[i]["U_Reg"].ToString(), TempTable.Rows[i]["T_Reg"].ToString());
                        }
                        TempTable.Dispose();
                    }
        ...
        public void ShowGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                MessageBoxFa.Show($"پرسنل: {ShowGridView.CurrentRow.Cells[1].Value} {ShowGridView.CurrentRow.Cells[2].Value}\n تاریخ: {ShowGridView.CurrentRow.Cells[4].Value}\n مدت: {ShowGridView.CurrentRow.Cells[5].Value} ساعت\n  مبدا: {ShowGridView.CurrentRow.Cells[6].Value}\n توضیحات: {ShowGridView.CurrentRow.Cells[7].Value}", "جزئیات اضافه کار");
            }
        }
