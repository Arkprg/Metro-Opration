...
namespace Metro_Operation
{
    ...
    public partial class HistoryPostForm
    {
        public HistoryPostForm()
        {
            InitializeComponent();
        }
        ...
        public void Hist(string P_Num, ref string U_Reg, ref string T_Reg)
        {
            using (OleDbConnection StrConnec=new OleDbConnection(AppState.ConnectionString))
            {
                StrConnec.Open();
                using (OleDbCommand CMD=new OleDbCommand("Select Person.Fname, Person.Family, Person.P_Num, PostHis.Shift_name, PostHis.Shift_Time, PostHis.Shift_Loc, PostHis.P_Staff, PostHis.P_Post, PostHis.U_Reg, PostHis.T_Reg From PostHis INNER JOIN Person ON Person.P_Num=PostHis.P_Num WHERE Person.P_Num='" + P_Num + "'", StrConnec))
                using (OleDbDataReader Reader=CMD.ExecuteReader())
                {
                    while (Reader.Read())
                    {
                        ShowGridView.Rows.Add(j + 1, Reader["Fname"].ToString(), Reader["Family"].ToString(), Reader["P_Num"].ToString(), Reader["Shift_Loc"].ToString(), Reader["Shift_Time"].ToString(), Reader["Shift_name"].ToString(), Reader["P_Staff"].ToString(), Reader["P_Post"].ToString(), U_Reg, T_Reg);
                        U_Reg=Reader["U_Reg"].ToString();
                        T_Reg=Reader["T_Reg"].ToString();
                        j++;
                    }
                }
            }
        ...
        public void ShowButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    StrConnec.Open();
                    string StrQuer="SELECT DISTINCT PostHis.P_Num, Person.Fname, Person.Family, Person.Shift_Loc, Person.Shift_Time, Person.Shift_name, Person.P_Staff, Person.P_Post From PostHis INNER JOIN Person ON Person.P_Num=PostHis.P_Num WHERE Person.Vis=True";
                    ...
                    //AND PostHis.Tarikh BETWEEN '" & StartCalendar.Text & "' AND '" & EndCalendar.Text & "'
                    ...
                    using (OleDbDataReader Reader=CMD.ExecuteReader())
                    {
                        while (Reader.Read())
                        {
                            U_Reg="--";
                            T_Reg="--";
                            Hist(Reader["P_Num"].ToString(), ref U_Reg, ref T_Reg);

                            ShowGridView.Rows.Add(j + 1, Reader["Fname"].ToString(), Reader["Family"].ToString(), Reader["P_Num"].ToString(), Reader["Shift_Loc"].ToString(), Reader["Shift_Time"].ToString(), Reader["Shift_name"].ToString(), Reader["P_Staff"].ToString(), Reader["P_Post"].ToString(), U_Reg, T_Reg);
                            j++;
                            ShowGridView.Rows.Add("", "", "", "", "", "", "", "", "", "", "");
                        }
                    }
                }
                ...
                {
                    ShowGridView.Rows.RemoveAt(ShowGridView.Rows.Count - 1);
                }
        ...
        public void HistoryPostForm_Load(object sender, EventArgs e)
        {
            try
            {
