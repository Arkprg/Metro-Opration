...
namespace Metro_Operation
{
    ...
    public partial class DailyLeaveReportForm
    {
        public DailyLeaveReportForm()
        {
            InitializeComponent();
        }
        ...
        public void DailyLeaveReportForm_HelpButtonClicked(object sender, System.ComponentModel.CancelEventArgs e)
        {
            Help.ShowHelp(this, AppState.DatabasePath + "\\Help\\help.chm", HelpNavigator.TopicId, "040501");
        }

        public void Form32_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Button6_Click(object sender, EventArgs e)
        {
            ...
            ShowForm.WebBrowser1.Navigate(MainForm.Path + "\\Help\\Hlp32.mht");
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (OleDbConnection StrConnec=new OleDbConnection(AppState.ConnectionString))
                    {
                        ...
                        string CommandStr="SELECT Person.Fname, Person.Family, Person.P_Num, Morakhasi.Tarikh, Morakhasi.Kind, Morakhasi.U_Reg, Morakhasi.T_Reg From Morakhasi INNER JOIN Person ON Person.P_Num=Morakhasi.P_Num WHERE Morakhasi.Vis=True AND Morakhasi.Tarikh BETWEEN '" + StartCalendar.Text + "' AND '" + EndCalendar.Text + "' ORDER BY Morakhasi.Tarikh DESC, Person.Family, Person.Fname";
                        OleDbDataAdapter Adap=new OleDbDataAdapter(CommandStr, StrConnec);
                        Adap.Fill(DataT);

                        int i;
                        for (i=0; i <= DataT.Rows.Count - 1; i++)
                        {
                            ShowGridView.Rows.Add(i + 1, DataT.Rows[i]["Fname"].ToString(), DataT.Rows[i]["Family"].ToString(), DataT.Rows[i]["P_Num"].ToString(), DataT.Rows[i]["Tarikh"].ToString(), DataT.Rows[i]["Kind"].ToString(), DataT.Rows[i]["U_Reg"].ToString(), DataT.Rows[i]["T_Reg"].ToString());
                        }

                    }

                    Panel1.Enabled=true;
                    Label3.Text=(DateTime.Now.Hour * 3600 + DateTime.Now.Minute * 60 + DateTime.Now.Second - t).ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        public void ShowButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string StrQuer="Select Person.Fname, Person.Family, Person.P_Num, Morakhasi.Tarikh, Morakhasi.Kind, Morakhasi.U_Reg, Morakhasi.T_Reg From Morakhasi INNER JOIN Person ON Person.P_Num=Morakhasi.P_Num WHERE Morakhasi.Vis=True";

                    if (NameBox.Text.Length > 0)
                    {
                        StrQuer += " AND Morakhasi.P_Num='" + PNum + "'";
                    }
                    else
                    {
                        if (AppState.UserLevel > 1)
                        ...
                        {
                            StrQuer += " AND Person.P_Staff<>'مدیریت'";
                        }
                    ...
                    StrQuer += " AND Morakhasi.Tarikh BETWEEN '" + StartCalendar.Text + "' AND '" + EndCalendar.Text + "' ORDER BY Morakhasi.Tarikh DESC, Person.Family, Person.Fname";
                    ...
                    using (OleDbConnection StrConnec=new OleDbConnection(AppState.ConnectionString))
                    {
                        ...
                        {
                            ShowGridView.Rows.Add(i + 1, TempTable.Rows[i]["Fname"].ToString(), TempTable.Rows[i]["Family"].ToString(), TempTable.Rows[i]["P_Num"].ToString(), TempTable.Rows[i]["Tarikh"].ToString(), TempTable.Rows[i]["Kind"].ToString(), TempTable.Rows[i]["U_Reg"].ToString(), TempTable.Rows[i]["T_Reg"].ToString());
                        }
                    }
