...
namespace Metro_Operation
{
    ...
    public partial class TripSpecialReportForm
    {
        public TripSpecialReportForm()
        {
            InitializeComponent();
        }
        ...
        public void TripDetailReportForm_HelpButtonClicked(object sender, System.ComponentModel.CancelEventArgs e)
        {
            Help.ShowHelp(this, AppState.DatabasePath + "\\Help\\help.chm", HelpNavigator.TopicId, "040602");
        }

        public void TripDetailReportForm_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (OleDbConnection StrConnec=new OleDbConnection(AppState.ConnectionString))
                    {
                        ...
                        string QueStr="SELECT Person.Fname, Person.Family, Person.P_Num, Person.Line_Num, TakTrip.Tarikh, TakTrip.T_Shift, TakTrip.Trip, TakTrip.U_Reg, TakTrip.T_Reg FROM TakTrip INNER JOIN Person ON Person.P_Num=TakTrip.P_Num WHERE TakTrip.Vis=True";
                        if (NameBox.Text.Length > 0)
                        {
                            QueStr += " And Person.P_Num='" + PNum + "'";
                        }
                        else
                        {
                            if (AppState.UserLevel > 1)
                        ...
                        QueStr += " AND TakTrip.Tarikh BETWEEN '" + StartCalendar.Text + "' AND '" + EndCalendar.Text + "' ORDER BY TakTrip.Tarikh, Person.Family, Person.Fname";
                        ...
                        using (OleDbDataReader Reader=CMD.ExecuteReader())
                        {
                            ...
                            {
                                i++;
                                ShowGridView.Rows.Add(i, Reader["Fname"].ToString(), Reader["Family"].ToString(), Reader["P_Num"].ToString(), Reader["Tarikh"].ToString(), Reader["Trip"].ToString(), Reader["U_Reg"].ToString(), Reader["T_Reg"].ToString());
                            }
                        }
                    }
