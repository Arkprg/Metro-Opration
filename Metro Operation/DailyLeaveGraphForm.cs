...
{
    public partial class DailyLeaveGraphForm
    {
        public DailyLeaveGraphForm()
        {
            InitializeComponent();
        }
        ...
        public void ShowButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Chart1.Titles[0].Text="نمودار تعداد مرخصی روزانه از تاریخ " + StartCalendar.Text + " تا " + EndCalendar.Text;
                ...
                else
                {
                    using (WaitForm wait=new WaitForm())
                    {
                        ...
                        {
                            StrConnec.Open();
                            string StrComm="SELECT COUNT(Morakhasi.Tarikh) AS Cou From Morakhasi INNER JOIN Person ON Person.P_Num=Morakhasi.P_Num WHERE Morakhasi.Vis=True";

                            if (NameBox.Text.Length > 0)
                            {
                                StrComm += " AND Morakhasi.P_Num='" + PNum + "'";
                            }
                            else
                            {
                                if (AppState.UserLevel > 1)
                            ...
                            {
                                PersianDate.AddDay(1);
                                using (OleDbCommand CMD=new OleDbCommand(StrComm + " AND Morakhasi.Tarikh='" + PersianDate.Tarikh() + "'", StrConnec))
                                {
                                    Cou="0";
                                    using (OleDbDataReader Reader=CMD.ExecuteReader())
                                    {
                                        while (Reader.Read())
                                        {
                                            Cou=Reader["Cou"].ToString();
                                        }
                                    }

                                }

                                Chart1.Series[0].Points.AddXY(PersianDate.Tarikh().Substring(5, 5), Cou);
                            }
                        }
        ...
        public void DailyLeaveGraphForm_Load(object sender, EventArgs e)
        {
            try
            {
