...
namespace Metro_Operation
{
    ...
    public partial class FunctionReportForm
    {
        public FunctionReportForm()
        {
            InitializeComponent();
        }
        ...
        public void Form15_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (OleDbConnection StrConnec=new OleDbConnection(AppState.ConnectionString))
                    {
                        ...
                        string StrQuer="Select Person.Fname, Person.Family, Person.P_Num, Amal.Tarikh, Amal.OnvAmal, Amal.Amal, Amal.U_Reg ,Amal.T_Reg ,Amal.Mem From Amal INNER JOIN Person ON Person.P_Num=Amal.P_Num WHERE Amal.Vis=True And (Amal.OnvAmal='منفی' OR Amal.OnvAmal='مثبت')";
                        if (AppState.UserLevel > 5 || AppState.UserLevel > 15)
                        {
                            StrQuer += " AND Amal.U_Reg='" + AppState.UserName + "'";
                        }

                        if (NameBox.Text.Length > 0)
                        {
                            StrQuer += " And Amal.P_Num='" + PNum + "'";
                        }
                        else
                        {
                            if (AppState.UserLevel > 1)
                            ...
                            if ((string)LocalCombo.SelectedItem != "همه موارد" && AppState.UserLevel < 6)
                            {
                        ...
                        StrQuer += " AND Amal.Tarikh BETWEEN '" + StartCalendar.Text + "' AND '" + EndCalendar.Text + "' ORDER BY Amal.Tarikh DESC, Person.Family, Person.Fname";
                        ...
                        {
                            ShowGridView.Rows.Add(i + 1, TempTable.Rows[i]["Fname"].ToString(), TempTable.Rows[i]["Family"].ToString(), TempTable.Rows[i]["P_Num"].ToString(), TempTable.Rows[i]["Tarikh"].ToString(), TempTable.Rows[i]["OnvAmal"].ToString(), TempTable.Rows[i]["Amal"].ToString() + " (" + TempTable.Rows[i]["Mem"].ToString() + ")", TempTable.Rows[i]["U_Reg"].ToString(), TempTable.Rows[i]["T_Reg"].ToString());
                        }
        ...
        public void FunctionReportForm_HelpButtonClicked(object sender, System.ComponentModel.CancelEventArgs e)
        {
            Help.ShowHelp(this, AppState.DatabasePath + "\\Help\\help.chm", HelpNavigator.TopicId, "040401");
        }
    }
}
