...
namespace Metro_Operation
{
    ...
    public partial class TripImportFinalForm
    {
        public TripImportFinalForm()
        {
            InitializeComponent();
        }
        ...
        public bool ImportChecker(string TripDay)
        {
            ...
            {
                StrConnec.Open();
                using (OleDbCommand CMD=new OleDbCommand("SELECT Tarikh FROM DailyTripReg WHERE Tarikh='" + TripDay + "' AND T_Type='لوحه نهایی' AND Vis=True", StrConnec))
                {
        ...
        public void RegButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ErrorShow(DateLabel, "لوحه نهایی" + DateLabel.Text + "ثبت شده است");
                }
                else
                {
                    ...
                    using (WaitForm wait=new WaitForm())
                    {
                        ...
                        using (OleDbConnection StrConnec=new OleDbConnection(AppState.ConnectionString))
                        {
                            ...
                            {
                                if (TripDate != TripGridView.Rows[i].Cells[1].Value.ToString())
                                {
                                    TripTable.Clear();

                                    string ss="SELECT * FROM DailyTrip WHERE Vis=True AND Execu=True AND Tarikh='" + TripGridView.Rows[i].Cells[1].Value.ToString() + "'";
                                    OleDbDataAdapter Adap=new OleDbDataAdapter(ss, StrConnec);
                                    Adap.Fill(TripTable);
                                    Adap.Dispose();
                                    TripDate=TripGridView.Rows[i].Cells[1].Value.ToString();
                                }

                                DataRow[] DRow=TripTable.Select("Tarikh='" + TripGridView.Rows[i].Cells[1].Value.ToString() + "' AND T_Time='" + TripGridView.Rows[i].Cells[2].Value.ToString() + "' AND Mabdae='" + TripGridView.Rows[i].Cells[3].Value.ToString() + "' AND Maghsad='" + TripGridView.Rows[i].Cells[4].Value.ToString() + "' AND O1_NUM='" + TripGridView.Rows[i].Cells[6].Value.ToString() + "' AND O2_NUM='" + TripGridView.Rows[i].Cells[10].Value.ToString() + "' AND OT_NUM='" + TripGridView.Rows[i].Cells[8].Value.ToString() + "'");

                                if (DRow.Length > 0)
                                {
                                    using (OleDbCommand CMD=new OleDbCommand("UPDATE DailyTrip SET Final=True WHERE ID=" + DRow[0][0], StrConnec))
                                    {
                                        CMD.ExecuteNonQuery();
                                    }
                                }
                                ...
                                {
                                    using (OleDbCommand CMD=new OleDbCommand("INSERT INTO DailyTrip (Tarikh, T_Time, Mabdae, Maghsad, O1_NUM, O2_NUM, OT_NUM, Final, Vis) VALUES ('" + TripGridView.Rows[i].Cells[1].Value.ToString() + "','" + TripGridView.Rows[i].Cells[2].Value.ToString() + "', '" + TripGridView.Rows[i].Cells[3].Value.ToString() + "', '" + TripGridView.Rows[i].Cells[4].Value.ToString() + "', '" + TripGridView.Rows[i].Cells[6].Value.ToString() + "', '" + TripGridView.Rows[i].Cells[10].Value.ToString() + "', '" + TripGridView.Rows[i].Cells[8].Value.ToString() + "', True, True)", StrConnec))
                                    {
                                        CMD.ExecuteNonQuery();
                                    }
                                }
                            ...
                            using (OleDbCommand CMD=new OleDbCommand("INSERT INTO DailyTripReg (Tarikh, T_Type, U_Reg, T_Reg, Vis) VALUES ('" + DateLabel.Text + "','لوحه نهایی', '" + AppState.UserName + "', '" + MainForm.PersianToday.Tarikh() + " " + DateTime.Now.ToString("HH:mm:ss") + "', True)", StrConnec))
                            {
                                CMD.ExecuteNonQuery();
                            }
                        }
